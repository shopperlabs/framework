<div
    x-data="{
        options: ['general', 'dhl'],
        words: {'general': '{{ __("General") }}', 'dhl': '{{ __("DHL") }}'},
        currentTab: 'general'
    }"
>
            ...
            <div class="flex space-x-3 md:absolute md:top-3 md:right-0">
                <span class="shadow-sm rounded-md">
                    <button wire:click="launchModale" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-700 active:bg-blue-700 transition duration-150 ease-in-out">
                        {{ __("Create custom shipping method") }}
                    </button>
                </span>
            </div>
        </div>
        <div>
            ...
            <div class="hidden sm:block">
                <nav class="-mb-px flex space-x-8">
                    ...
                    <button @click="currentTab = 'dhl'" type="button" class="whitespace-no-wrap pb-4 px-1 border-b-2 border-transparent font-medium text-sm leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300" :class="{ 'border-blue-500 text-blue-600 focus:text-blue-800 focus:border-blue-700': currentTab === 'dhl' }">
                        {{ __("DHL") }}
                    </button>
                </nav>
            </div>
    ...
    <div class="mt-10">
        <div x-show="currentTab === 'general'">
            <div class="mt-6 bg-white shadow sm:rounded-md">
                <div class="p-4 sm:p-6 sm:pb-4">
                    <label for="filter" class="sr-only">{{ __('Search carriers') }}</label>
                    <div class="flex rounded-md shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <x-shopper-input.text id="filter" wire:model.debounce.300ms="search" class="rounded-none rounded-md pl-10" placeholder="{{ __('Search carrier by name') }}" />
                                    ...
                                    <tr>
                                        <td class="px-6 py-3 max-w-0 w-full whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
                                            <div class="flex items-center space-x-3 lg:pl-2">
                                                <div class="flex-shrink-0 w-2.5 h-2.5 rounded-full {{ $carrier->is_enabled ? 'bg-green-600': 'bg-gray-400' }}"></div>
                                                <div class="flex items-center">
                                                    @if($carrier->logo_url)
                                                        <img class="h-8 w-8 rounded object-cover object-center" src="{{ $carrier->logo_url }}" alt="">
                                                    @else
                                        ...
                                        <td class="px-6 py-3 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                            <div class="flex items-center">
                                                <span x-data="{ on: @if($carrier->is_enabled) true @else false @endif }" role="checkbox" tabindex="0" wire:click="toggleStatus({{ $carrier->id }}, {{ $carrier->is_enabled ? 1 : 0 }})" x-on:toggle-saved-{{ $carrier->id }}.window="on = !on" @keydown.space.prevent="on = !on" :aria-checked="on.toString()" aria-checked="false" @focus="focused = true" @blur="focused = false" class="group relative inline-flex items-center justify-center flex-shrink-0 h-5 w-10 cursor-pointer focus:outline-none">
                                        ...
                                        <td class="hidden md:table-cell px-6 py-3 whitespace-no-wrap text-sm leading-5 text-gray-500">
                                            @if($carrier->link_url)
                                                <a href="{!! starts_with($carrier->link_url, ['http://', 'https://']) ? $carrier->link_url : "https://{$carrier->link_url}" !!}" target="_blank" class="inline-flex items-center text-gray-500 hover:text-gray-400 font-medium text-sm leading-5">
                                                    {{ $carrier->link_url }}
                                        ...
                                        <td class="hidden md:table-cell px-6 py-3 whitespace-no-wrap text-sm leading-5 text-gray-500 text-right">
                                            <time datetime="{{ $carrier->created_at->format('Y-m-d') }}" class="capitalize">{{ $carrier->created_at->formatLocalized('%d %B, %Y') }}</time>
                                        </td>
                                        <td class="pr-6">
                                            <div x-data="{ open: false }" x-on:item-update.window="open = false" @keydown.escape="open = false" @click.away="open = false" class="relative flex justify-end items-center">
                                                ...
                                                <div x-show="open" x-description="Dropdown panel, show/hide based on dropdown state." x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="mx-3 origin-top-right absolute right-7 top-0 w-48 mt-1 rounded-md shadow-lg" style="display: none;">
                                                    <div class="relative z-10 rounded-md bg-white shadow-xs" role="menu" aria-orientation="vertical" aria-labelledby="project-options-menu-0">
                                                        <div class="py-1">
                                                            <button wire:click="modalEdit({{ $carrier->id }})" wire:key="{{ $carrier->id }}" type="button" class="group w-full flex items-center px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" role="menuitem">
                                                        ...
                                                        <div class="py-1">
                                                            <button wire:click="removeCarrier({{ $carrier->id }})" type="button" class="group flex w-full items-center px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" role="menuitem">
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-3 whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
                                            <div class="flex justify-center items-center space-x-2">
                                                ...
                                                <span class="font-medium py-8 text-cool-gray-400 text-xl">{{ __("No shipping methods") }}...</span>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                ...
                <div class="bg-white px-4 py-3 flex items-center rounded-b-md justify-between border-t border-gray-200 sm:px-6">
                    <div class="flex-1 flex justify-between sm:hidden">
                        {{ $carriers->links('shopper::livewire.wire-mobile-pagination-links') }}
                    </div>
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm leading-5 text-gray-700">
                                {{ __('Showing') }}
                                <span class="font-medium">{{ ($carriers->currentPage() - 1) * $carriers->perPage() + 1 }}</span>
                                {{ __('to') }}
                                <span class="font-medium">{{ ($carriers->currentPage() - 1) * $carriers->perPage() + count($carriers->items()) }}</span>
                                {{ __('of') }}
        ...
        <div x-cloak x-show="currentTab === 'dhl'">
            <livewire:shopper-settings.carriers.dhl />
        </div>
    </div>
    ...
    <x-shopper-modal wire:model="display" maxWidth="lg">
            ...
            <div class="sm:flex sm:items-start px-4 sm:px-6 py-4">
                <div class="text-left">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">
                        {{ $carrierId ? __("Update shipping method") : __('Add new shipping method') }}
                    </h3>
                </div>
            </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    @if(! $logoUrl)
                        <div class="sm:col-span-2">
                            <div>
                                <x-shopper-label value="{{ __('Carrier Logo') }}" />
                                <div class="mt-2">
                    ...
                    @endif
                    <div class="sm:col-span-2">
                        <x-shopper-input.group label="Custom carrier name" for="name" :error="$errors->first('name')" isRequired>
                            <x-shopper-input.text wire:model="name" id="name" />
                        </x-shopper-input.group>
                    </div>
                    <div class="sm:col-span-2">
                        <x-shopper-input.group label="Carrier Website Documentation" for="link_url">
                            <x-shopper-input.text wire:model="linkUrl" type="url" id="link_url" placeholder="https://doc.myprovider.com" />
                        </x-shopper-input.group>
                    </div>
                    <div class="sm:col-span-2">
                        <x-shopper-input.group label="Additional details" for="description" helpText="Displays to customers when they’re choosing a shipping method.">
                            <x-shopper-input.textarea wire:model="description" id="description" />
                        </x-shopper-input.group>
                    </div>
                    <div class="sm:col-span-2">
                        <x-shopper-input.group label="Payment instructions" for="instructions" helpText="Displays to customers after they place an order with this shipping method.">
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <span class="flex w-full sm:ml-3 sm:w-auto">
                @if($carrierId)
                    <x-shopper-button wire:click="updateShippingMethod" type="button" wire.loading.attr="disabled">
                        <svg wire:loading wire:target="updateShippingMethod" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                        </svg>
